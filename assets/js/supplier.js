...
var url;
		function newData(){
			...
			url = 'modules/supplier/save_supplier.php';
		}
		function editData(){
			...
			if (row){
				...
				url = 'modules/supplier/update_supplier.php';
			}
		}
		function saveData(){
			$('#fm').form('submit',{
		...
		function removeData(){
			...
			if (row){
				$.messager.confirm('Confirm','Anda yakin akan menghapus data ini?',function(r){
					if (r){
						$.post('modules/supplier/remove_supplier.php',{id_supplier:row.id_supplier},function(result){
							if (result.success){
								$('#dg').datagrid('reload');	// reload the user data
							} else {
